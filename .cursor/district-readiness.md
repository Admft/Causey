...
13. **P4 · Chess federation pin was missing · M · shipped 2026-09-05**  
   Surface: `/chess` results and the homepage chess rail now have a labeled “Get your kid to chess nationals” placement into `/pathways`. Home is a wide local → state → nationals rail; `/chess` is the same climb as a compact strip. Not an official US Chess ruling; swap copy when they send written pathway info.
