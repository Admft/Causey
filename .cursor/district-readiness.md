...
- Problem reports from `/support` (optional screenshot) email the founding team; replies can land in Alerts; bots are refused; platform admins can close the queue in bulk
...
13. **P4 · Chess federation pin was missing · M · shipped 2026-09-05**  
   Surface: `/chess` results and the homepage chess rail now have a labeled “Get your kid to chess nationals” placement into `/pathways`. Home is the split card (red promise + white Local/State/Nationals path); `/chess` is the red face only. Not an official US Chess ruling; swap copy when they send written pathway info.
