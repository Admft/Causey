...
- **Parent assign next:**
  - **Parent/ops first:** Active priority 1 — apply through `0046`, run §6 + §7 smoke, log results. (Assigned to human — agents will not fake env apply.)
  - **UI (`kimi-k3-max`):** Active priority 2 ✅ shipped. Idle unless smoke finds a recovery-copy defect.
  - **Backend (`gpt-5.6-sol-high`):** Active priority 3 — idle unless smoke/env apply fails; then fix the defect. Do not start P2 work.
