...
## Done
- [x] 2026-08-08 — Scrape outbound accuracy: every feed now retains an event-specific destination when one exists instead of a generic registration homepage. US Chess resolves exact, year-aware Squarespace event pages and pulls covers only from those pages; CCA stays on its event detail because ChessAction exposes only a homepage; dedupe prefers a lower-priority source's more specific registration page + cover. Live cleanup: 23 Texas Chess Center event links with 22 distinct covers, 50 CCA homepage links replaced, 364 TLA homepage links replaced, 125 repeated homepage covers removed, and 86 unrecoverable legacy links cleared rather than misdirecting
...
- Business constraint (2026-08-08): no partner districts may be added to the site until letters of intent are signed; a longer partner list is expected end of August 2026, with current partners concentrated in LA/NYC and slow replies while contacts are on vacation. Product posture: keep zero named partner districts, logos, or counts anywhere public — the "assisted pilot" framing and Illustrative-labeled preview on `/districts` + home are the correct ceiling until LOIs land. When they do, provisioning is an ops task, not a build task: platform-admin district creation (`0025`), assisted-pilot runbook (`0034`), pending-by-default school verification.
