...
- [x] Season-board previews promoted from loose rows to full mini app screens that fill the pane (user call: sketches too small/empty): window chrome, join-link field + member rows + dashed invite slot, event cards with date chips + "Host a tournament" slot, day-of checklist, results rows with blank place cells + the real "Blanks mean not recorded." note, provision tree with a pending school, school tournament with the coach's day-of roster, 5×2 connected-school grid, aggregate bar table — 2026-08-26
