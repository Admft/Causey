...
- [x] Organizer band objects, second pass: the club scoresheet is a real sheet (header rule, continuous red margin line down the numeral column, stamped field-label chip per step, fine-print colophon) and the district panel holds an office report (District office header with Aggregate totals only stamp, schematic week calendar — hollow squares for school tournaments, one full-width bar for the district-wide event — with a real legend) instead of two plastic cells — 2026-08-26
