...
- [x] Home coverage is two lanes instead of a three-step path: an open ledger of the five directories (row links into each, one red tick per indexed source under a “Sources” legend, names as plain text) beside a closed soft panel of directories Causey may not index, grouped by category. Drops 33 outbound links, the six chess logos that made one row look finished and four look broken, and the bespoke line-draw in favour of the shared `ScrollReveal`; section is 668px instead of 761px and holds at 360/768/1280/1440 — 2026-08-26
