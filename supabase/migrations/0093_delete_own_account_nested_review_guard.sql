-- 0091 guarded organization_verification_reviews with to_regclass, but put
-- the EXISTS check in the same IF expression. PL/pgSQL plans that whole
-- expression at once, so a missing table still raises 42P01 and the phone
-- Settings delete path shows "needs a database update".
-- Nest the EXISTS so the relation is only planned when the table exists.
...
declare
  actor uuid := auth.uid();
  owned_name text;
  has_review_history boolean := false;
begin
  ...
  if to_regclass('public.organization_verification_reviews') is not null then
    ...
    into has_review_history;
    if has_review_history then
