-- Zip search with no category was counting custom/other published events in
-- total_count, then JS dropped them — inflated totals and short pages.
-- Match the non-radius path: discovery categories only when p_category is null.
...
as $$
  ...
  filtered as (
    ...
    where c.status = 'published'
      ...
      and (p_category is not null or c.category in ('chess', 'debate', 'stem', 'arts', 'writing'))
...
comment on function public.search_competitions_in_radius(
  ...
  'Pages published competitions within radius using earthdistance; includes online events without coordinates. Unscoped searches are limited to public discovery categories.';
